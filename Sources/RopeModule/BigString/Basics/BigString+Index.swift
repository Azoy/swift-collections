...
extension BigString.Index {
  ...
  /// The specific index for the chunk. Only valid if `_rope` is not nil.
  internal var _chunkIndex: BigString._Chunk.Index {
    get {
      assert(_rope != nil)
      return BigString._Chunk.Index(UInt16(truncatingIfNeeded: _rawBits & 0x7FF))
    }

    set {
      assert(newValue.flags <= 0x7)
...
extension BigString.Index {
  ...
  internal func _knownScalarAligned() -> Self {
    var copy = self
    copy._chunkIndex = _chunkIndex.knownScalarAligned()
    return copy
  }

  internal func _knownCharacterAligned() -> Self {
    var copy = self
    copy._chunkIndex = _chunkIndex.knownCharacterAligned()
    return copy
  }
  ...
  public init(_utf8Offset: Int, utf16TrailingSurrogate: Bool) {
    ...
    if utf16TrailingSurrogate {
      _chunkIndex.setIsUTF16TrailingSurrogate()
    }
  }
...
extension BigString {
  func resolve(_ i: Index, preferEnd: Bool) -> Index {
    ...
    return Index(
      baseUTF8Offset: i.utf8Offset - Int(ci.utf8Offset),
      _rope: ri,
      chunk: ci)
  }
}
