...
extension BigString {
  func _distance(
    ...
    if ropeIndexA == ropeIndexB {
      ...
      d -= metric.distance(from: chunkA.utf8.startIndex, to: chunkIndexA, in: chunkA)
      d += metric.distance(from: chunkB.utf8.startIndex, to: chunkIndexB, in: chunkB)
    }
...
extension BigString {
  func _index(
    ...
    assert(distance <= 0)
    assert(ci == chunk.utf8.startIndex)
    let start = ri
    _rope.formIndex(&ri, offsetBy: &d, in: metric, preferEnd: false)
    chunk = _rope[ri]
    ci = metric.index(at: d, in: chunk)
...
extension BigString {
  ...
  func _utf8Index(after i: Index) -> Index {
    ...
    var ci = i._chunkIndex
    let chunk = _rope[ri]
    chunk.utf8.formIndex(after: &ci)
    if ci == chunk.utf8.endIndex {
      return Index(
...
extension BigString {
  func _character(at start: Index) -> (character: Character, end: Index) {
    ...
    let endOffset = start._chunkIndex.utf8Offset + char.utf8.count
